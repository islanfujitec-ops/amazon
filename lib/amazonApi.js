...
function mapItem(item) {
  ...
  return {
    ...
    store: listing?.merchantInfo?.name || 'Amazon.com.br',
    emEstoque: listing?.availability?.type === 'IN_STOCK',
...
// Varios ASINs numa chamada so (a API aceita ate 10 por requisicao).
async function getItemsByASINs(asins) {
  const lista = (asins || []).slice(0, 10);
  if (!lista.length || !CREDENTIAL_ID || !CREDENTIAL_SECRET || !PARTNER_TAG) return [];
  const token = await getOAuth2Token();
  if (!token) return [];
  try {
    const response = await axios.post(
    ...
    return (response.data?.itemsResult?.items || []).map(mapItem);
  } catch (error) {
    console.log(`⚠️ Erro getItems (${error.response?.status}): ${error.message}`);
    return [];
  }
}

// Diagnóstico: retorna o erro REAL de cada etapa (sem fallback), para depuração.
async function debugApi(keywords = 'Catan', resObj = null) {
  ...
  try {
    const payload = {
      ...
      resources: resObj || ['itemInfo.title', 'offersV2.listings.price']
    };
...
module.exports = { searchAmazonProducts, getProductByASIN, getItemsByASINs, debugApi };
